import {
    ShifterClient, DEFAULT_ENDPOINT,
} from '../src/client'
...
describe('client.ts', () => {
    ...
    describe('URL generation', () => {
      ...
      it('should return callable API URL with string', () => {
        ...
        expect(c.getURL('custom')).toEqual("https://api.hl.getshifter.net/custom")
      })
      it('should return callable API URL with dirty string', () => {
        const c = new MockClass({
            token: 'DUMMY'
        })
        expect(c.getURL('/custom//xxx-xxx-xx/')).toEqual("https://api.hl.getshifter.net/custom/xxx-xxx-xx")
      })
    })
    })
