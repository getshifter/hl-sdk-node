...
export class OAuth2Process {
    ...
    public async getIssuer (): Promise<Issuer<Client>> {
        const issuer = await Issuer.discover(
            `${this.config.issuer}/${this.config.userPoolId}`
        )
        return issuer
    }
...
const config = {
    ...
    clientId: '7pbq6f7ah4a783c9c5uqs5r0bb',
    redirectUris: [
        'http://localhost:3000/cb'
    ],
    clientSecret: '1o3jgbe6bpm23oqtu7ed1d2v4fu0qqqc1bteei4derl3una8cfka',
...
export async function getToken () {
    const oauth2 = new OAuth2Process(
        config
    )
